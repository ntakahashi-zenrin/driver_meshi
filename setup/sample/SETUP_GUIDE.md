...
エラーなく完了したら、Appwriteコンソールの「Databases」を開いてコレクションが7種類（drivers・meal_posts・gallery_posts・follows・likes・comments・notifications）作成されていることを確認してください。

---

## STEP 6：フロントエンドの接続先を設定する

フロントエンド（Webアプリ）を起動する前に、「どのAppwriteに接続するか」を教えるファイルを1つ作ります。  
`web/` フォルダの中に `.env.local` というファイルを新規作成し、以下の内容を書いて保存してください：

```
VITE_APPWRITE_ENDPOINT=http://サーバーのIPアドレス/v1
...
> このファイルはGitで管理されていません（`.gitignore` 対象）。  
> サーバー上だけに存在するファイルなので、開発者のPC環境には影響しません。
...
## STEP 8：フロントエンドを起動して動作確認する

サーバー上で `web/` フォルダに移動してフロントエンドを起動します：
...
> `--host` をつけることで、サーバー上で動いているアプリに別のPCのブラウザからアクセスできるようになります。

起動後、ブラウザで `http://サーバーのIPアドレス:5173` を開きます。  
